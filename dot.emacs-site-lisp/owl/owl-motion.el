;;; owl-motion.el --- a part of the simple OWL mode for Xemacs
...
(defvar *owl-move-cross-levels* nil "If T then fwd, back motions will cross embedding levels.")

(defvar *owl-definition-matcher* nil "If non-nil, then the definition match string")
(make-variable-buffer-local '*owl-definition-matcher*)

(defvar *owl-definition-indent* nil "If non-nil, then the definition indent column.")
(make-variable-buffer-local '*owl-definition-indent*)

(defvar *def-begin-pattern* "<[^!/?]")

(defun owl-definition-matcher ()
  (or *owl-definition-matcher*
      (let ((str *def-begin-pattern*))
	(dotimes (i (owl-definition-indent))
	  (setq str (concat " ?" str)))
	(setq *owl-definition-matcher* (concat "^" str)))))


(defun owl-definition-indent ()
  (or *owl-definition-indent*
      (let ((curpos (point)))
	(beginning-of-buffer)
...
;  (owl-def-begin-string *owl-definition-indent*)

(defun owl-beginning-of-def-p ()
  (let ((curpos (point))
	ok)
    (backward-char (owl-definition-indent))
    (setq ok (looking-at (owl-definition-matcher)))
    (goto-char curpos)
    ok))
...
(defvar owl-search-no-error t)

;;; search forward or return OWL-specific error provided
(defun search-forward-or-err (string &optional errstr)
  (let ((pt (point))
	(res (search-forward string nil owl-search-no-error)))
    (if res res
      (error (or errstr (format "Syntax Error: Missing %s after %s" string pt))))))
	
;;; search backward or return OWL-specific error provided
(defun search-backward-or-err (string &optional errstr)
  (let ((pt (point))
	(res (search-backward string nil owl-search-no-error)))
    (if res res
...
;;; search forward for a regexp, skipping quoted strings and balanced xml and owl/rdf comments
...
(defun owl-search-fwd (regexp &optional pt-to-beginning) 
...
;;; search backward for a regexp, skipping quoted strings and balanced xml and owl comments
;;; skip quoted strings and xml comments. Must start outside an xml comment.
;;; start-prev is how many chars before (point) to start looking-at.
(defun owl-search-bkwd (regexp &optional start-prev)
...
(defun owl-read-token (&optional pos move)
  (let* ((curpos (point))
	 (beg (or pos (point)))
	 (tok nil))
    (goto-char beg)
...
(defun owl-beginning-of-comment-if-inside (&optional move)
  (let ((curpos (point))
...
(defun owl-goto-tag-begin (&optional backup-if-outside)
  (interactive)
  (let* ((curpos (point))
	 pos
...
;(define-key owl-mode-map (keystroke '((control c) ?b)) 'owl-goto-tag-begin)

;;;;;;;;;;;;; OWL-MOVE-FORWARD, BACKWARD
...
;;; if before an end, then dont move unless *owl-move-cross-levels*
;;; as this is a level change. 
;;; should return position if moved?
(defun owl-forward-tag (&optional n) 
...
(defun owl-skip-tag-group (&optional tag)
  (unless tag (setf tag (owl-read-token)))
  (let* ((curpos (point))
...
(defun owl-find-matching-tag-end (regexp)
  (let ((cnt 1) (matchpt t)) 
    (while (and (> cnt 0) matchpt)
      (setf matchpt (owl-search-fwd regexp t)) ;; set point at beginning of match
...
;;; and *owl-move-cross-levels* is NIL (handled in OWL-GOTO-TAG-BEGIN)
...
(defun owl-backward-tag (&optional n)
	...
	(progn 
	  (while (> n 0) 
	    ...
	    (let ((empty-endp nil)
	      ...
	      ;; if after tag (owl-goto-tag-begin 'back) goes directly AFTER the preceding > (if any)
	      ;; owl-goto-tag-begin returns (char . pos) if moved where char is ?> or ?<
	      (when (setf back (owl-goto-tag-begin 'back))
;		(dbg "after tag-begin back= %s looking-at %s pt= %s" back (bs 5) (point))
		(when (and back (eq (car back) ?>))
		  ...
		  ;; which would mean changing levels (only do it if *OWL-MOVE-CROSS-LEVELS*)
		  (cond ((looking-back-at "-->")
			 (setq newpos (search-backward "<!--" nil t)))
			((looking-back-at "/>")
			 (setf empty-endp t)
;;			 (message "empty-end-tag")
			 (setq newpos (owl-search-bkwd "<")))
			;; else a regular tag or comment end
			(t (setq newpos (owl-search-bkwd "<" ))))
...
;		(dbg "after tag-back looking at: %s emptyend? %s back=%s" (bs 10) empty-endp back)
		...
		(cond ((null back) (error "No prior tag"))
		      ...
		      ((looking-at "</") (owl-skip-back-tag-group))
		      ;; else some other begin tag. (looking-at "<[^/]")
		      ;; stay in front of it if an empty begin, 
		      ;; else go back to original location unless *owl-move-cross-levels*
;		      ((progn (dbg "past </ test") nil))
		      ((or empty-endp 
			   *owl-move-cross-levels*
			   (not newpos))
...
(defun owl-skip-back-tag-group (&optional tag)
  (unless tag (setf tag (owl-read-token)))
  ;; strip off the /
  (if (char= (elt tag 0) ?/)
  ...
  (let* ((curpos (point))
    ...
    (owl-find-matching-tag-begin tag-regexp)))

(defun owl-in-empty-begin-tag-p ()
  (let ((pt (point)))
...
;;; regexp is the tag to search for
(defun owl-find-matching-tag-begin (regexp)
  (let ((cnt 1) (matchpt t)) 
    (while (and (> cnt 0) matchpt)
      (setf matchpt (owl-search-bkwd regexp))
      ;; no match is an error (unbalanced tags)
      (cond ((null matchpt) (error "Couldn't find matching begin tag %s" regexp))
	    ...
	    (t 
	     (unless (owl-in-empty-begin-tag-p)
		 (decf cnt)))))
    matchpt))
...
(defun owl-up-tag () 
  ...
  (let ((*owl-move-cross-levels* nil)
	(curpos (point))) ;; should be its default value anyway
    (while (owl-backward-tag));; find begining of this level
    (or (owl-search-bkwd "<[^/]")
	(progn
	  (goto-char curpos)
	  nil))))
...
(defun owl-up-tag-end ()
  "Move up an embedding level to past the enclosing end </TAG>."
  (interactive "_")
  ;; make sure backward-tag doesn't cross levels, then go as far as it can.
  (let ((*owl-move-cross-levels* nil)
	(curpos (point))) ;; should be its default value anyway
    (while (owl-forward-tag));; find end of this level
    (or (owl-search-fwd "</[^>]*>") ;; go after next end tag
	(progn
	  (goto-char curpos)
	  nil))))
...
(defun owl-move-after-begin-tag ()
  (let ((curpos (point)))
    (when (looking-at "<[^/]")
      (if (and (owl-search-fwd ">")
	       (not (looking-back-at "/>")))
	  (point)
	(goto-char curpos)
	nil))))
...
;;; else stay put
(defun owl-inside-tag () 
  "Find the first tag with content at current tag embedding level and move inside it."
  (interactive "_") 
  (let ((*owl-move-cross-levels* nil) ;; should be its default value anyway
	(curpos (point))
	(downpos nil))
    (when (owl-goto-tag-begin)
      ;; now check if a non-empty begin, or else repeat by trying to go forward tag 
      (while (and (not (setq downpos (owl-move-after-begin-tag)))
		  (owl-forward-tag)))
      (if downpos downpos
	  (goto-char curpos)
	  nil)
...
If already at a definition start, move before the previous one, if any."
  (interactive "_") 
  (let ((curpos (point))
	(atbeg (owl-goto-tag-begin)))
    (cond ((null atbeg) ;; at or near eof, so no following open bracket
	   (if (re-search-backward "<" nil t)
	       (cond ((looking-at "</rdf:RDF>") (owl-backward-tag))
		     ((looking-at "</")
		      (owl-search-fwd ">")(owl-backward-tag))) ;; this branch never tested?
	     (goto-char curpos)))
	  ((looking-at "<rdf:RDF>")(owl-backward-tag))
	  ;; moved up to next tag, so back up one if begin
	  ((and (< (point) curpos)
		(owl-beginning-of-def-p)))
	  ;; else moved to 
	  (t 
	   (if (= curpos (point))(owl-backward-tag))
	   (while (not (or (bobp) 
...
(defun owl-next-tag-def () 
  "Move to before the next definition, as approximated by an unindented begin tag."
  (interactive "_")
  (let ((curpos (point))
	res atbegin)
    ;; get to next tag beginning
    (when (setq atbegin (owl-goto-tag-begin))
      (cond ((<= (point) curpos) 
	     (owl-forward-tag)
	     (setq atbegin (owl-goto-tag-begin)))))	    
    (cond ((owl-beginning-of-def-p)
	   (while (not (or (eobp)
			   (null atbegin) 
			   (looking-at "</rdf:RDF")
			   (owl-beginning-of-def-p)))
...
(defvar owl-indent-increment 2 "Number of spaces to indent an RDF subexpression.")
...
(defun owl-indent-tag-column ()
  "Find column to indent to for a line of OWL code, with respect to the previous tag beginning." 
  (let* ((curpos (point))
	 (samelev t)
	 refcol)
;    (dbg (bs 6))
	...
	(let ((beg (owl-goto-tag-begin 'back)))
	  ...
	  (while (and beg
	    ...
	    (setq beg (owl-goto-tag-begin 'back))
	    )
;	  (dbg "beg=%S pt=%s" beg (point))
	  (cond ((null beg) (beginning-of-line))
		((char= (car beg) ?>) ;; found a prior tag end
		 ...
		 (cond
		  ...
		  (t 
		   (owl-search-bkwd "<")
		   (cond ((looking-at "</") (owl-up-tag) (current-column))
			 (t 
	...
	(re-search-backward 
	 ...
	 nil t)
	(current-column))
       ((looking-at "</") (owl-up-tag) (current-column))
       ((looking-at "<") 
	(cond ((owl-backward-tag) (current-column))
	      ((owl-up-tag)
	       (setq samelev
...
;	(dbg "in a tag pos=%s %s" (point) (buffer-substring (point) (+ 5 (point))))
	(setq samelev nil) (owl-goto-tag-begin) (current-column))))
    ;;; now we're ready to say the column
    (if samelev refcol (+ refcol owl-indent-increment))))
...
(defun owl-indent-line ()
  "Indent a line of OWL code, with respect to the previous tag beginning."  
  (interactive)
  (let* ((pos (- (point-max) (point)))
	 (textpos (owl-non-white-line-pos))
;	 (textcol (current-column))
	 (samelev t)
	 (refcol (owl-indent-tag-column)))
;    (dbg (bs 6))
...
(defun line-clear-before-pt ()
  (let* ((curpos (point))
	 (res t)
	 )
    (backward-char)
    (while (and (> (point) 0)(not (looking-at "^")) res)
      (cond ((looking-at "[^ \t]")
	     (setq res nil))
	    (t (backward-char))))
    (goto-char curpos)
...
(defun owl-open-new-line-for-insert ()
  (let ((curpos (point)))
...
(defun owl-newline ()
  "Do a newline at point, then indent for OWL readability."
  (interactive)
  (newline)
  (owl-indent-line))
...
;;; INDENT-EXPRESSION
(defun owl-indent-expression ()
  "Indent each line of the tag expression that begins on the current line."
  (interactive)
  (beginning-of-line)
  (let* ((curpos (point))
	 (textpos (owl-non-white-line-pos))
	 (numlines (count-lines curpos (progn (owl-forward-tag) (point)))))
...
;      (dbg "looking at %s" (bs 10))
    ...
    (goto-char curpos)
    (owl-non-white-line-pos)
    ))


;;; for testing
; (defun xx ()(interactive) (owl-search-bkwd "</?rdf:Property"))
