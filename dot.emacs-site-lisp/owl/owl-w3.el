;;; owl-w3.el --- a part of the simple OWL mode for Xemacs
...
(defvar owl-w3-loadedp nil)

(cond ((locate-library "w3")
       (require 'w3)
       (setq owl-w3-loadedp t)))

;;; adaptation/overlay on w3 package for OWLITE mode

(defvar *owl-url-buffer-map* nil 
  "assoc list of buffer names and the URLs that were loaded into them")

(defvar *owl-in-buffer-quietly* t 
  "If finding id in loaded buffer, don't ask.") 

;;; cons of buffer name and (point)
(defvar *owl-last-position* nil 
...
(defun owl-url-new-buffer (url buf) 
  ;; remove any deleted buffers
  (setf *owl-url-buffer-map*
    (remove-if (function (lambda (pr) (buffer-deleted-p (cdr pr))))
	       *owl-url-buffer-map*))
  (let ((buffer (get-buffer buf)))
    (when buffer
      (push (cons url buffer) *owl-url-buffer-map*))))

;;; see if the url (up through file name) is in a buffer
(defun owl-url-already-loaded (url)
  (when url
    (let* ((cell (assoc url *owl-url-buffer-map*))
	   (buf (and cell (cdr cell))))
      ;; if buffer was deleted (or renamed), remove the pointer
      (cond ((and cell (or (not buf) (not (buffer-live-p buf))))
	     (setf *owl-url-buffer-map* (delete as *owl-url-buffer-map*))
	     nil)
	    (t buf)))))

;;; *owl-url-buffer-map is (url . buffer)*
;;; get the url of a buffer (reverse assoc)
(defun owl-buffer-url (bufname)
  (let ((buf (get-buffer bufname)))
    (dolist (cell *owl-url-buffer-map*)
...
(defvar *owl-xmlns-map* nil
  "Assoc list RDF xmlns names to their expansions.")

(make-variable-buffer-local '*owl-xmlns-map*)
(set-default '*owl-xmlns-map* nil)

;;; substitutions gleaned from <ENTITY name "expansion"> within DOCTYPE tag at beginning of file
(defvar *owl-entity-map* nil 
  "Assoc list of xml entity names to their expansions")

(make-variable-buffer-local '*owl-entity-map*)
(set-default '*owl-entity-map* nil)

(defun owl-entity-expansion (name)
  (let ((cell (assoc name *owl-entity-map*)))
     (when cell (cdr cell))))
    
;;; Given an XML namespace reference (NAME), return the expansion according to an
;;; XMLNS=line in the RDF tag, previously cached in *OWL-XMLNS-MAP*
;;; Do &ENTITY; expansion before returning the result.
;;;  For now, also strip off the ending # if there is one.

(defun owl-rdfns-expansion (name &optional no-remove-lb)
  (let* ((cell (assoc name *owl-xmlns-map*))
	 (url (and cell (cdr cell)))
	 numpos)
    (setf uri (owl-expand-entname url))
...
;;; store mappings in *OWL-ENTITY-MAP* for this buffer

(defun owl-extract-entity-map ()
  (interactive)
  (let ((entmap nil))
    (beginning-of-buffer)
...
(defun owl-build-entity-map (&optional forcep)
  (interactive)
  (when (or forcep (null *owl-entity-map*))
    (let* ((emap (owl-extract-entity-map))
	   (this (assoc "THIS" emap)))
      (setq *owl-entity-map* emap)
      (if (and this (null (owl-buffer-url (current-buffer))))
	  (owl-url-new-buffer (cdr this) buf))
      )))

;;; given a string, expand using ENTITY substitutions
(defun owl-expand-entname (tok-string)
  (let* ((tok tok-string)
	 p1)
    (while (setq p1 (position ?& tok))
      (let* ((p2 (and p1 (position ?\; tok :start p1)))
	     (subst-name (and p2 (substring tok (1+ p1) p2)))
	     (replacement (owl-entity-expansion subst-name)))
        (if replacement 
	    (setf tok
...
;;; store mappings in *OWL-XMLNS-MAP* for this buffer
;;; buf is a string (buffer name) 
(defun owl-build-rdfns-map (&optional forcep)
  (interactive)
  (when (or forcep (null *owl-xmlns-map*))
    (let* ((emap (owl-extract-rdfns-map))
	   (this (or (assoc "THIS" emap)
		     (assoc "this" emap)))
	   (buf (current-buffer)))
      (setq *owl-xmlns-map* emap)
      (if (and this (not (owl-buffer-url buf)))
	  (owl-url-new-buffer (cdr this) buf))
      emap
      )))
...
(defun owl-parse-prolog (&optional forcep)
  (owl-build-entity-map forcep)
  (owl-build-rdfns-map forcep))

;;; We need to break up and expand xmlns refs like owl:class based on the rdf xmlns attributes
...
;;; Break up a token and return a OWL-NAME tuple
;;; where namespace is the prefix before : (except http:)
;;; where uri-base is the URI PATH up to filename
;;; where filename is the local filename (e.g. "foo.owl")
;;; and where name is what follows the # in an expanded URI
;;; if an expanded URI (starts with http) then namespace is nil

(defun owl-parse-rdf-token (tok-string  &optional type no-subst buf)
  (owl-parse-prolog) ;; if wasnt done before
  (unless no-subst
    (setf tok-string (owl-expand-entname tok-string)))
  ...
  (let* ((pos (position ?: tok-string))
    ...
    (cond ((string-equal (upcase ns) "HTTP")
	  ...
	  (pos ;; a non HTTP with colon
	   (setq uri (owl-rdfns-expansion ns)) ;; expand the prefix
	  ...
	  ((member type '(ATTNAME ENTNAME)) ; NO prefix, just a name in the default namespace
	   (setf target tok-string uri (owl-rdfns-expansion nil)) ; the default xmlns
	   )
	  (t (setf target tok-string uri nil))) ;; a name in current buffer?
    (when (or target uri)
      (make-owl-name :target target :uri uri :namespace ns :type type :source tok-string))))
...
;;; use this instead. 
(eval-and-compile
  (defvar owl-get-token-chars "-.a-zA-Z0-9_/:~&#;" ;; took out ()=%?@ 
    "Valid characters in a URL")
  )

;;; Note, this includes dbl-quote in the list of things to ignore when searching back
(defvar owl-space-chars "[ \t\n\]")

(defvar owl-nontok-chars "[] \t\n\"]")

(defmacro owl-backup-over-whitespace ()
    `(skip-chars-backward owl-space-chars))
...
(defun owl-look-before (pt match-elts)
  (let ((old-pt (point))
	(ok t))
    (goto-char pt)
...
(defmacro owl-char-before (pt pat)  
  `(owl-look-before ,pt '("[ \t\n]" * ,pat)))
...
;;; returns (cons <found token string> <parsed form as a OWL-NAME>)
(defun owl-token-at-or-before (&optional pt buf)
	...
	(cond ((or (next-char-is ?<) (between-chars ?< ?/))
	  ...
	  ;; skip / if begin of end tag.
		 ...
		 (skip-chars-backward "/>"))
	       ...
	       (skip-chars-backward owl-get-token-chars)
...
;	(dbg "%s start %s.%s" toktype (char-before) (char-after))
	(skip-chars-forward owl-get-token-chars)
	(setq end (point))
;	(dbg "%s end %s.%s" toktype (char-before) (char-after))
	...
	(when (null toktype)
	  (setf toktype
		(cond ((looking-at "[ \t\n]*=") 'ATTNAME)
		      ((owl-look-before start '(?\" "[ \t\n]" * ?=) ) 'REFERENCE)
		      ((owl-char-before start "[</]") 'ENTNAME)
		      ((owl-char-before start ">") 'VALUE)
		      (t nil))))
;	(list tok toktype)
;	  (dbg "found token %s" (list tok toktype))
	(owl-parse-rdf-token tok toktype)
	)))


(defun owl-name-to-string (tup &optional no-target) 
  (let ((base (owl-name-uri tup))
	(target (owl-name-target tup)))
...
(defun owl-filename-parts (fname &optional no-target)
...
(defun owl-make-url-string (type host path &optional target)
...
(defun owl-file-found-locally (url &optional buf)
...
(defun owl-read-url-with-default (&optional tuple)
  (interactive) ;; interactive for testing only
  (let ((in-owl-buffer (eq 'owl-mode major-mode))
	tup)
    (setf tup
	  (cond (tuple)
		;; TUP is a OWL-NAME tuple
		(in-owl-buffer (owl-token-at-or-before))
		(t (make-owl-name :source ""))))
  (let* ((url (owl-name-uri tup)) ;; the base uri up to target
	 (target (owl-name-target tup))
	 (in-buf (cond ((null url) (if in-owl-buffer (current-buffer)))
		       (t (owl-url-already-loaded url))))
	 (locally (when (and (not in-buf) url) 
		    (owl-file-found-locally url)))
	 (default (or (and tup (owl-name-to-string tup))
...
;      (dbg "Note= %S" note)
    ...
    (cond ((and confirm (not (equal confirm default)))
	   ...
	   (setf confirm (owl-find-confirmation-replacement confirm)) 
	   (owl-read-url-with-default (owl-parse-rdf-token confirm nil)))
	   ;; if the base url path is local or in a buffer, say so unless *owl-in-buffer-quietly*
	   ((and in-buf 
		 (or *owl-in-buffer-quietly* 
...
(defun owl-find-confirmation-replacement (str)
...
;;; (local directory if it was used for another URI with the same basepath)
;;; (upto filename)
...
(defun owl-find-def-in-buffer (target &optional buf)
  (interactive "sBuffer Find URI:")
  (if buf (set-window-buffer (selected-window) buf)
    (owl-remember-position) ; if called stand-alone, work in this buffer, remember position.
...
;;; because we are trying to do this asynchronously, 
;;; the post-retrieval actions are done by
;;; owl-post retrieval, rather than here. 

(defun owl-view-url (url)
  (interactive (list (owl-read-url-with-default)))
  (owl-remember-position)
;  (trace-msg "URL is %s" url)
  (cond ((listp url)
	 (case (car url)
	   (BUFFER (owl-find-def-in-buffer (third url) (second url)))
	   (FILE (find-file (second url))
		 (owl-mode) ;; in case not a .owl extension
		 (owl-find-def-in-buffer (third url) nil))))
	((and (stringp url) 
	      owl-w3-loadedp)
	 (owl-view-url-page url))
	(t (message "W3 Package not loaded. Only local files available.")
	)
  ))

  
(defun owl-remember-position () (interactive)
    (setf *owl-last-position* (cons (buffer-name (current-buffer)) (point))))

(defun owl-goto-previous-position () (interactive)
  (let* ((newpos *owl-last-position*)
...
;;;;;;;;;;;;;;;;  WEB-RETRIEVE AND VIEW A OWL NAME or RESOURCE reference ;;;;;;;;;;;;;;;
;;; The result of a lot of digging: fn URL-RETRIEVE (url.el) calls
;;; URL-RETRIEVE-INTERNALLY (w3/url.el) that is the driver. 
;;; This funcalls var HANDLER which is usu URL-HTTP
;;; (based on URL extension or default "auto")
...
;;; URL-SENTINEL funcall's the var URL-DEFAULT-RETRIEVAL-PROC 
;;; which = 'URL-DEFAULT-CALLBACK
...
;;; W3-PASS-TO-VIEWER proceeds to rename the temp buffer 
;;; using the non-directory part of the url
;;; and put it in fundamental mode (at least for owl files which are of type text/plain)

;;; For type text/http, however, (as returned with an error page 
;;; generated by most servers)
;;; we get different behavior. In this case, MM-MIME-INFO acts like this
;;;  mm-mime-info: st="text/html; charset=iso-8859-1" nd=nil request=t
;;;  <- mm-mime-info: (("viewer" . w3-prepare-buffer) 
;;;    ("test" fboundp (quote w3-prepare-buffer)) ("type" . "text/html"))
;;; and W3-PREPARE-BUFFER is funcalled by W3-PASS-TO-VIEWER. 
;;; This *DELETES* the original buffer
;;; and spits the parse'd text into a new buffer, using the HTML commands in the original.

;;;;; ANALYSIS:

;;; For the purposes of owl files, which should be plain text, 
;;; we can rely on the current buffer 
...
;;; and keep track of which buffer names correspond to each url, 
;;; so we don't go generating them over 
...
(defvar *owl-last-retrieval-url* nil "url that was focus of last retrieval")

;;; This deals just with retrieving the web page to a buffer
(defun owl-view-url-page  (url)
  ...
  (let* ((old-asynch (default-value 'url-be-asynchronous))
    ...
    (setf *owl-last-retrieval-url* url)
    (when pos (setf url (substring url 0 pos))) ;; remove target in file
    (unwind-protect
	(progn
	  (setq-default url-be-asynchronous t)
	  (save-excursion
	    (setf retr  (owl-url-retrieve url)) ;; was url-retrieve
	    ))
      (setq-default url-be-asynchronous old-asynch))
    ))


(defun owl-check-http-header ()
  (beginning-of-buffer)
...
(defun owl-strip-off-http-header ()
  (beginning-of-buffer)
...
(defvar *owl-last-retrieval-status* nil "(status . buffer) when set by asynch retrieval")
...
(defun owl-url-retrieve (url &optional no-cache expected-md5)
...
be honored before this function exits."
  ...
  (let ((status (owl-retrieve-text-url url no-cache)))
    status))

;;; copied from recent version of url.el (for xemacs)
(defun owl-get-url-working-buffer-name ()
...
(defun owl-retrieve-text-url (url &optional no-cache)
  (let* ((url-working-buffer (owl-get-url-working-buffer-name))
...
;;; This variable is buffer local, so don't need to worry about resetting it. 
      ...
      (setq url-current-callback-data nil
	    url-current-callback-func 'owl-w3-callback)
      ) ; end save-excursion
...
(defun owl-w3-callback (&optional proc string)
	  ...
	  (when url-be-asynchronous ;; should be T here (MB)
	    (cond
	     ...
	     ((url-mime-response-p) 
	      ...
	      (setf *owl-last-retrieval-status* (cons stat (current-buffer)))
;;;	      (scratch-msg "in callb %S" *owl-last-retrieval-status*)
	      )) ;; buffer local
...
;;;	  (scratch-msg "\n before continuation buf=%s" (current-buffer))
	  (let ((x (w3-build-continuation))
		(url (url-view-url t)))
	    (when (and *owl-last-retrieval-status*
		       (= (car *owl-last-retrieval-status*) 200))
...
(defun owl-make-buffer-url-visible ()
...
(defun owl-set-window-buffer (buf)
...
(defun owl-post-retrieval ()
  (let* ((buf (cdr *owl-last-retrieval-status*))
	 (stat (car *owl-last-retrieval-status*))
	 (prevbuf (car *owl-last-position*))
;;;      (prevbufpt (cdr *owl-last-position*))
	 (pos (position ?# *owl-last-retrieval-url*))
	 (uri (if pos (substring *owl-last-retrieval-url* 0 pos) 
		*owl-last-retrieval-url*))
	 (new-buf-name (file-name-nondirectory uri))
	 (target (if pos (substring *owl-last-retrieval-url* (1+ pos) nil)))
	 )
    (when buf
      (set-buffer buf)
      (setq stat (or stat 
...
;;;      (dbg "post retrieval stat=%s" stat)
      (cond ((eq stat 200)
	     ...
	     (owl-make-buffer-url-visible)
	     (owl-mode)
	     (toggle-read-only 1)
;;;	     (beginning-of-buffer)
	     (owl-url-new-buffer uri buf) ;; store ref to new owl buffer
	     (if target (owl-find-def-in-buffer target nil)))
...
;;;    (scratch-msg "\nin post-retr buf= %S stat=%S\n" (current-buffer) *owl-last-retrieval-status*)
    ;; for last line of url-sentinel
    (owl-set-window-buffer buf)
...
;;;	      (scratch-msg "in url-sent/async")
		...
		(when (boundp 'buffer-original-url) 
		  (setf owl-retrievalp t)
		  (setf *owl-last-retrieval-status*
...
;;;		(scratch-msg "url-sent wb= %S status %S" (current-buffer) status)
    ...
    ;; if a owl-retrieval, force the current buffer to be the found one
    ;; (overrides a save-excursion and save-window-excursion in url-default-callback)
    (if (and owl-retrievalp *owl-last-retrieval-status*)
	(owl-set-window-buffer (cdr *owl-last-retrieval-status*))
